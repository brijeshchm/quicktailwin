...
<div class="animate-fade-in space-y-4 md:space-y-6"
 ...
 @if($overdue)<span class="rounded bg-destructive px-1.5 py-0.5 text-white">{{ $overdue }} Overdue</span>@endif
</button>
     </div>
    ...
    >
        ...
        >
            ...
            >
                ...
                <form
                    :action="'/leads/' + followup + '/follow-ups'"
                    method="POST"
                    id="followup-form"
                    class="space-y-4"
                    @submit.prevent="
                ...
                >
                    ...
                    <div class="grid gap-4 sm:grid-cols-2">
                        ...
                        <div class="flex items-end gap-2">
                            ...
                            <button
                                type="button"
                                id="applyFollowUpDateBtn"
                                onclick="applyFollowUpDate()"
                                class="btn btn-primary h-[42px] shrink-0"
                            >
                                Apply
                            </button>
...
window.enquiryController = {
    ...
    async storeFollowUp(leadId, form) {
        ...
        try {
            ...
            const response = await fetch(`/leads/${leadId}/follow-ups`, {
                method: 'POST',
                body: formData,
                headers: {
    ...
    async getAllFollowUps(leadId, limit = 5) {
        ...
        try {
            ...
            const res = await fetch(
                `/leads/${leadId}/follow-ups/list?limit=${encodeURIComponent(limit)}`,
                {
                    headers: {
                        'X-Requested-With': 'XMLHttpRequest',
                        'Accept': 'application/json'
