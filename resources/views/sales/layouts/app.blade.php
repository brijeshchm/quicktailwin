...
<body class="bg-[#f7f9fb] text-[#12263a]">
    <div class="min-h-screen">
        ...
        <aside data-mobile-menu class="fixed inset-y-0 left-0 z-40 hidden w-[248px] flex-col bg-[#12263a] px-5 py-6 lg:flex">
            <a href="{{ route('admin.dashboard') }}" class="flex items-center gap-3 px-2"><span class="grid h-9 w-9 place-items-center rounded-xl bg-[#f0b45a] text-xl font-bold text-[#12263a]">V</span><span class="font-display text-lg font-semibold tracking-tight text-white">vendor<span class="text-[#f0b45a]">flow</span></span></a>
            <p class="mt-12 px-2 text-[10px] font-semibold uppercase tracking-[0.18em] text-[#6f879a]">Workspace</p>
            <nav class="mt-3 space-y-1">
                <a href="{{ route('admin.dashboard') }}" class="{{ request()->routeIs('admin.dashboard') ? 'bg-[#1b3a52] text-white' : 'text-[#aabccc]' }} block rounded-xl px-3 py-2.5 text-sm font-medium hover:bg-[#1b3a52] hover:text-white">Dashboard</a>
                <a href="{{ route('admin.vendors.index') }}" class="{{ request()->routeIs('admin.vendors.*') ? 'bg-[#1b3a52] text-white' : 'text-[#aabccc]' }} flex items-center justify-between rounded-xl px-3 py-2.5 text-sm font-medium hover:bg-[#1b3a52] hover:text-white"><span>Vendors</span><span class="rounded-full bg-[#f0b45a] px-1.5 py-0.5 text-[10px] font-bold text-[#12263a]">{{ \App\Models\Vendor::count() }}</span></a>
