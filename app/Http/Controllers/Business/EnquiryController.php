...
class EnquiryController extends Controller
{
		...
		if ($request->pauseLead == '1') {
		 ...
		 $client->save();
		  ...
		  return response()->json([
				'status'    => true,
				'pauseLead' => (int) $client->pauseLead,
				'message'   => $client->pauseLead
	...
	public function scrapLead(Request $request)
	{
		...
		if (!empty($assignedLead)) {
			...
			$assignedLead->scrapValue = $request->scrap_value;
	...
	public function readLead(Request $request)
	{
		...
		if ($assignedLead->save()) {
			return response()->json(['status' => true, 'message' => 'Read lead updated']);
		} else {
			return response()->json(['status' => false, 'message' => 'Not Read lead updated']);
		}
	}
	...
	public function favoritleads(Request $request)
	{
		...
		if ($assignedLead->save()) {
			return response()->json(['status' => true, 'message' => 'Favorite lead updated']);
		} else {
			return response()->json(['status' => false, 'message' => 'Favorite lead updated']);
		}
	}
	/**
